...
$crc->ensureIndex(array('psiid' => 1), array('unique' => 1));
$cgc->ensureIndex(array('id' => 1), array('unique' => 1));
...
{
        //var_dump($i);
	$base->update(array("PSIID"=>$i["PSIID"]),array('$set'=>array("flag"=>"1")));	
}

if($_SESSION["admin"]=="HRA")
header('location:hradminsettings.php?status=Inserted successfully');        
elseif($_SESSION["admin"]=="HRM")
header('location:adminsettings.php?status=Inserted successfully');        
