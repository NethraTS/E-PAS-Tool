...
$ext = substr(strrchr($fileName, "."), 1);
	...
	if ($ext != "xls") {
	header('location:goaladminsettings.php?status="Please upload .xls format. Try Again!!!"'); 
	exit;
	}
	...
	if ($fileSize > 0 && $fileSize <= MAX_FILE_SIZE) {
		...
		if (!$result) {
			...
			header('location:goaladminsettings.php?status="Error uploading File"'); 
			exit;
		}else {
...
{
	header('location:goaladminsettings.php?status="Review cycle has already begun.Cannot upload now."');        
	echo $pod["Date"];
...
foreach($body as $i)
{
	...
	try{
		...
		header('location:creategoalcollections.php?status=libinserted');        
	}catch(MongoException $mongoException){
		header('location:goaladminsettings.php?status="Duplicate Records are not inserted."');        
