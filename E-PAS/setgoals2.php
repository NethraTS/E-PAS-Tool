...
<?php 
$gstatus=array("Q3 2015","Q4 2015","Q1 2016","Q2 2016");
$eid=$_GET['eid'];
$doc=$c->findOne(array("id"=>$eid));
	if($doc["flag"]=="0")
	{}	
...
for($i=0;$i<course_choices.length;$i++)
{
	...
	{	  
	  if(dw>0&&inc1<=11)
	  alert("Deliverable not set");
	   else if(qw>0&&inc2<=21)
	  alert("Quality not set");
	   else if(cw>0&&inc3<=31)
	  alert("Initiatives/ Ownership not set");
	   else if(ocw>0&&inc4<=41)
	  alert("Organizational/ Project Contribution not set");
	   else if(vaw>0&&inc5<=51)
	  alert("Strategic Planning /Value Addition not set");
	   else if(vaw<=0&&inc5>51)
	  alert("Weightage for Strategic Planning /Value Addition not set");
	  else if(inc6<=61)
	  alert("Training not set");
	   else if(comm1=="none")
	  alert("Specify a name for training!!");
	 else 
	   {
...
if(cate=="train")
	{
 		$('table#training > tbody').append('<tr class="rows" id="t'+inc6+'"><td><input class="traint2" name="traint1" type="text" maxlength="100" id="'+cate+defaultTextAreaCount1+'" ></td><td><!--<input  class="datetimepickers" placeholder="Click here for date"  >--></td><td><select name="trainq" id="'+cate+defaultTextAreaCount1+'"><option>Q3 2015</option><option>Q4 2015</option><option>Q1 2016</option><option>Q2 2016</option></select></td><td><button class="btn btn-xs btn-info pull-right d" id="t'+dnc6+'" onclick="deletedesgn(this)"><i class="fa fa-times"></i>&nbspDelete</td></tr>');
		var todayDate="<?php echo date('Y/m/d'); ?>";
var todayTime="<?php echo date('H:i'); ?>";
var dateTime="<?php echo date('Y/m/d H:i') ?>";
